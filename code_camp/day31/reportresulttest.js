...
"ryan"이 "con"을 4번 신고했으나, 주어진 조건에 따라 한 유저가 
같은 유저를 여러 번 신고한 경우는 신고 횟수 1회로 처리합니다. 
따라서 "con"은 1회 신고당했습니다. 3번 이상 신고당한 이용자는 
...
/*
id_list 를 객체를 만들고 
다시 객체를 만들어 신고한 id 리스트 이중으로 만들어 수식화 할 수 있겠다.

처음 id_list에 신고한 리스트를 만들고,
그 리스트로 신고당한 리스트에 합을 구한 후
신고 횟수가 k 보다 넘어가는 리스트를 업!
...
function solution(id_list, report, k) {
  ...
  let reported = id_list.reduce((acc, cur) => {
    let arr = Object.keys(obj[cur]);
    for (key of arr) {
      if (key in acc) {
        acc[key]++;
      } else {
  ...
  let answer = id_list.map((x) => {
    let count = 0;
    let arr = Object.keys(obj[x]);
    for (key of arr) {
      if (reported[key] >= k) count++;
    }
    return count;
  });

...
/*
arr 요소들을 toString으로 이진수로 바꿔주고 
각 요소별로 1과 0으로 answer 요소를 만들어준 다음 replace를 통해서 #으로 변환 후
answer 요소 숫자를 삽입시켜준다.
...
function solution(n, arr1, arr2) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    answer.push(s.replace(/1/g, "#").replace(/0/g, " "));
  }
...
function solution(n, arr1, arr2) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let s = "";
    for (let j = 0; j < n; j++) {
      if (temp1[j] === "1" || temp2[j] === "1") s += "#";
      if (temp1[j] === "0" && temp2[j] === "0") s += " ";
    }

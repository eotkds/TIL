...
function solution(new_id) {
  ...
  let fourlevel = twolevel.replace(/\.{2,}/g, ".").replace(/^\.|\.$/g, ""); // 4단계 까지
  if (fourlevel === "") fourlevel = " "; //공백이 나오는 오류가 발생하여 수정
  let fivelevel = fourlevel.replace(" ", "a"); // 5단계

  let sixlevel = fivelevel.slice(0, 15); //6단계 시작
  if (sixlevel[14] === ".") sixlevel = sixlevel.replace(/\.$/g, ""); // 6단계 끝

  while (sixlevel.length <= 2) {
    sixlevel += sixlevel[sixlevel.length - 1];
  } // 7단계 검증
